...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    mask = torch.zeros_like(sim_matrix, device=device)
    eye = torch.eye(N,device=device)
    mask[:N,N:] = eye
    mask[N:,:N] = eye
    mask = mask.bool()

    pos_pair_sim = sim_matrix.masked_select(mask).view(2*N,1)
