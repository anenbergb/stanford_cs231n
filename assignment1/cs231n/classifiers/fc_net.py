...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        x_logits, cache2 = affine_forward(out1, self.params["W2"], self.params["b2"])

        if y is None:
            return x_logits
            # if we were instead supposed to return the softmax normalized probs
            # x_logits = x_logits - np.max(x_logits, axis=-1, keepdims=True)
            # x_prob = np.exp(x_logits)
            # x_prob /= np.sum(x_prob, axis=-1, keepdims=True)
            # return x_prob

        loss, d_softmax = softmax_loss(x_logits, y)

        # L2 regularization loss only applied to W1 and W2
        loss += 0.5 * self.reg * np.sum(self.params["W1"] * self.params["W1"])
        loss += 0.5 * self.reg * np.sum(self.params["W2"] * self.params["W2"])  

        grads = {}
        dout1, grads["W2"], grads["b2"] = affine_backward(d_softmax, cache2)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers):
            ...
            if i == self.num_layers - 1:
               outputs.append(fc_cache)
            else:
               x, relu_cache = relu_forward(x)
               outputs.append((fc_cache, relu_cache))
        ...
        # When using batch/layer nZormalization, you don't need to regularize the scale and shift parameters
        return loss, grads
